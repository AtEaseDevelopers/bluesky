    ...
    public function fetch_categories(Request $request)
    {
        ...
        if (!empty($records)) {
            foreach ($records as $record) {
                ...
                $nestedData['options'] = '<a href="' . route('admin.product-categories.edit', encrypt($record->id)) . '" class="btn btn-sm btn-primary me-1"><i class="fa fa-edit"></i></a><button type="button" class="btn btn-sm btn-danger btn-delete" data-bs-toggle="modal" data-bs-target="#delete" data-action="' . route('admin.product-categories.destroy', encrypt($record->id)) . '"><i class="fa fa-trash"></i></button>';
