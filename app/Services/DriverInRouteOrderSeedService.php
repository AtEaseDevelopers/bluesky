...
class DriverInRouteOrderSeedService
{
    ...
    public function run(?callable $logger = null): array
    {
        ...
        if ($customers->isEmpty()) {
            $log('No customers found. Create at least one customer first.');
        ...
        if (!$product) {
            $log('No products found. Create at least one product first.');
        ...
        $log("Done. Created {$created} in-route order(s), skipped {$skipped} driver(s).");
