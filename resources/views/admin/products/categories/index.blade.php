...
@section('title', __('product.categories.list'))
@section('css')
            ...
            <div class="card shadow no-border">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-4">
                        <h5 class="card-title">{{ __('product.categories.list') }}</h5>
                        <a href="{{ route('admin.product-categories.create') }}" class="btn btn-primary">
                            {{ __('product.categories.add') }}
                        </a>
                    </div>
                    <hr>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">{{ __('product.categories.delete') }}</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="{{ __('ui.close') }}"></button>
                </div>
                <div class="modal-body">
                    <p>{{ __('product.categories.delete_confirm') }}</p>
                </div>
